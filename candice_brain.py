...
Live signal direction is generated by asset class:
- REAL assets: AVWAP + Volume Profile + Bill Williams Alligator confirmation.
- OTC assets: PRO Structure Continuation (15M bias -> 1M BOS -> displacement
  -> retest -> hold -> confirmation candle).

The production scheduler, Telegram delivery and DEMO result watcher live in
app.py. The OTC strategy is isolated to assets explicitly identified as OTC;
real assets keep the existing AVWAP + Volume Profile brain.
"""
...
LIVE_STRATEGIES=(ALLOWED_STRATEGY,OTC_STRATEGY)
...
def _is_otc_asset(asset):
    """Return True only for assets explicitly labelled as OTC."""
    if not isinstance(asset,dict):
        return False
    values=[]
    for key in ("pair","symbol","instrument","title","display_name","displayName","name","mode","market_mode"):
        v=asset.get(key)
        if isinstance(v,str) and v.strip():
            values.append(v.strip().upper())
    for value in values:
        compact=re.sub(r"[^A-Z0-9]+","_",value).strip("_")
        if compact.endswith("_OTC") or value.endswith(" OTC"):
            return True
        if re.search(r"(^|[_\-\s])OTC($|[_\-\s])",value):
            return True
    return False
...
def _otc_body_ratio(c):
    return abs(float(c["close"])-float(c["open"]))/_otc_range(c)


def _otc_median(values):
    xs=sorted(float(x) for x in values if float(x)>0.0)
    if not xs:
        return 0.0
    n=len(xs)
    mid=n//2
...
def _otc_15m_bias(blocks):
    ...
    b1,b2,b3=blocks[-3:]
    bull_votes=sum((
        1 if float(b3["close"])>float(b2["close"]) else 0,
        1 if float(b3["high"])>=float(b2["high"]) and float(b3["low"])>=float(b2["low"]) else 0,
        1 if float(b2["close"])>float(b1["close"]) else 0,
    ))
    bear_votes=sum((
        1 if float(b3["close"])<float(b2["close"]) else 0,
        1 if float(b3["high"])<=float(b2["high"]) and float(b3["low"])<=float(b2["low"]) else 0,
    ...
    if bull_votes>=2 and bull_votes>bear_votes:
        return "BULLISH",bull_votes
    if bear_votes>=2 and bear_votes>bull_votes:
        return "BEARISH",bear_votes
    return None,max(bull_votes,bear_votes)
...
def analyze_otc_asset(
    ...
    """Analyze OTC assets only with the PRO Structure Continuation strategy."""
    forced=str(forced_strategy or "").upper().strip()
    if forced and forced!=OTC_STRATEGY:
        return None
    ...
    # activity gate can still require fresh observed tick activity at entry.
    ranges=[_otc_range(x) for x in cs[-8:-1]]
    latest_range=_otc_range(cs[-1])
    avg_recent=_otc_median(ranges)
    displacement_strength=min(
        1.0,
        float(setup["bos_range"])/max(float(setup["baseline_range"]),1e-12)
    )
    score=90.0
    score += min(4.0,float(max(0,bias_votes-1))*2.0)
    score += min(3.0,max(0.0,(displacement_strength-1.15)*4.0))
    score += 2.0 if setup["hold_confirmed"] else 0.0
    score += 2.0 if setup["confirmation_candle_confirmed"] else 0.0
    if avg_recent>0 and latest_range>=avg_recent:
        score += 1.0
    confidence=min(99,max(0,int(round(score))))
    ...
    features={
        "otc_strategy":True,
        "otc_market_bias":bias,
        "otc_bias_votes":int(bias_votes),
        "otc_bos_confirmed":True,
        "otc_bos_level":float(setup["bos_level"]),
        "otc_bos_candle_ts":int(setup["bos_candle_ts"]),
        "otc_displacement_confirmed":True,
        "otc_displacement_strength":round(displacement_strength,4),
        "otc_baseline_range":float(setup["baseline_range"]),
        "otc_bos_range":float(setup["bos_range"]),
        "otc_bos_body_ratio":round(float(setup["bos_body_ratio"]),4),
        "otc_retest_confirmed":True,
        ...
        "high_volume_gate_pending":True,
        "volume_gate_mode":"OTC_TICK_ACTIVITY_ENTRY_GATE",
        "value_position":"OTC_STRUCTURE",
        "value_area_acceptance":True,
        "level_reclaim":False,
        "slope_persistent":True,
        "profile_poc_migration_norm":0.0,
        "anchored_vwap":None,
        "volume_profile_poc":None,
        "volume_profile_vah":None,
        "volume_profile_val":None,
    ...
    return {
        ...
        "display_name":str(
            ...
            asset.get("name") or asset.get("pair") or ""
        ),
        "direction":direction,
...
def analyze_asset(
    ...
    """Return one deterministic asset-class-specific live candidate or None.

    OTC routing occurs before the REAL-asset AVWAP analysis below.
    """
    forced=str(forced_strategy or "").upper().strip()
    if forced and forced not in LIVE_STRATEGIES:
        return None
