...
+(o.amount??"—")+'</div>'+
       '<div class="meta">Entry '+(price??"—")+' • Live/Expiry '+(exitp??"—")+' • P/L '+(profit??"—")+'</div>'+
       '<div class="meta">Source '+(o._source||"broker_event")+'</div>'+
     '</div>';
   }).join("");
 }
 ...
 if(!ev.length){
   ...
   ee.innerHTML='<b>🛰️ BROKER ORDER EVENTS</b><pre>'+ev.map(e=>{
     const t=new Date(Number(e.received_at||0)*1000).toLocaleTimeString();
     return t+' • e:'+e.event+' • '+(e.pair||"—")+' • '+(e.direction||"—")+' • '+(e.status||"—")+' • '+(e.profit??"");
   }).join("\n")+'</pre>';
 }
